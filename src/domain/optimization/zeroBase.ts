...
import { VIX_WARNING, VIX_PANIC, NIKKEI_VI_WARNING, NIKKEI_VI_PANIC, SQ_BUFFER_DAYS_BEFORE } from '../risk/thresholds'
...
function deriveMarketMode(
  ...
  if (vix >= VIX_PANIC) reasons.push(`VIX ${vix.toFixed(1)} が暴落水準`)
  else if (vix >= VIX_WARNING) reasons.push(`VIX ${vix.toFixed(1)} が警戒水準`)

  if (nikkeiVI >= NIKKEI_VI_PANIC) reasons.push(`日経VI ${nikkeiVI.toFixed(1)} がパニック水準`)
  else if (nikkeiVI >= NIKKEI_VI_WARNING) reasons.push(`日経VI ${nikkeiVI.toFixed(1)} が高ボラ水準`)

  if (sqDays <= SQ_BUFFER_DAYS_BEFORE) reasons.push(`SQ直前（残り${sqDays}営業日）`)
  if (market.regime === 'bear') reasons.push('市場レジームがベア')

  const mode: MarketMode =
    vix >= VIX_PANIC || nikkeiVI >= NIKKEI_VI_PANIC
      ? 'emergency'
      : reasons.length > 0
      ? 'caution'
      : 'normal'
...
function fallbackUniverse(input: ZeroBaseInput): AssetCategorySummary[] {
  ...
  return [
    pack('JP_STOCK', '国内個別株', '上限管理', jpStockValue, input.jpStockMaxRatio ?? 0.10),
    pack('JP_TRUST', '国内株投信', '短期需給対応', jpTrustValue, 0.10),
...
function buildBuyProposals(
  ...
  // P4-A148: SAFE_MODE発動中・データ品質抑制中は新規個別株BUY提案を生成しない。
  // SELL/WAIT提案（buildSellProposals）は本関数の対象外のため引き続き生成される。
  if (baseBudget < 100_000 || mode === 'emergency' || input.safeModeActive || input.dqSuppressed) return []
  ...
  // currentJpStockValue when under cap (Math.min(cap, current)), so it cannot be used as cap.
  const currentJpStockValue = input.holdings.reduce((s, h) => s + h.eval, 0)
  const jpStockCap = totalAssets * (input.jpStockMaxRatio ?? 0.10)
  const safeJpStockHeadroom = Math.max(0, jpStockCap - currentJpStockValue)
  if (safeJpStockHeadroom <= 0) return []
  ...
  let remaining = roundToUnit(Math.min(baseBudget, safeJpStockHeadroom))
  ...
  for (let i = 0; i < scored.length; i += 1) {
    ...
    const maxPerPosition = Math.max(0, Math.min(totalAssets * (input.jpStockMaxRatio ?? 0.10), safeJpStockHeadroom))
