...
import {
  SHORT_BULL_VIX_MAX,
  SHORT_BULL_VIX_SOFT,
  SHORT_BULL_MIN_SQ_DAYS,
  SHORT_BULL_SOFT_SQ_DAYS,
  SHORT_BEAR_VIX_TRIGGER,
  SHORT_BEAR_VIX_SOFT,
  SHORT_BEAR_NIKKEI_VI_TRIGGER,
  SHORT_BEAR_NIKKEI_VI_SOFT,
...
// P4-A146: BUY予算按分専用の切り捨て丸め。四捨五入だと按分後の合計がheadroom上限を
// わずかに超過しうるため、BUY用途はfloorで安全側に倒す。
function floorToTenThousand(value: number) {
  return Math.max(0, Math.floor(value / 10_000) * 10_000)
}
...
function buildBullConditions(input: {
  ...
  const vixStableOrDown = input.vix <= SHORT_BULL_VIX_MAX && input.vixChg <= 0.2
  const vix = vixStableOrDown
    ? 'pass'
    : input.vix <= SHORT_BULL_VIX_SOFT && input.vixChg <= 0.8
    ? 'warn'
    : 'fail'
  const vi = input.nikkeiVIChg < 0 || input.spreadChg <= 0
    ...
    ? 'warn'
    : 'fail'
  const sq = statusFromValue(input.sqDays, SHORT_BULL_MIN_SQ_DAYS, SHORT_BULL_SOFT_SQ_DAYS, true)
...
function buildBearConditions(input: {
  ...
  const vix = input.vix >= SHORT_BEAR_VIX_TRIGGER || input.vixChgPct >= 5
    ? 'pass'
    : input.vix >= SHORT_BEAR_VIX_SOFT || input.vixChgPct >= 3
    ? 'warn'
    : 'fail'
  const vi = input.nikkeiVI >= SHORT_BEAR_NIKKEI_VI_TRIGGER || input.nikkeiVIChg > 0
    ? 'pass'
    : input.nikkeiVI >= SHORT_BEAR_NIKKEI_VI_SOFT || input.nikkeiVIChg > -0.15
    ? 'warn'
    : 'fail'
...
function calcBullConfidence(params: {
  ...
  const sqBonus = params.sqDays >= SHORT_BULL_MIN_SQ_DAYS ? 5 : params.sqDays >= SHORT_BULL_SOFT_SQ_DAYS ? 2 : 0
...
function calcBearConfidence(params: {
  ...
  const vixBonus = params.vix >= SHORT_BEAR_VIX_TRIGGER ? 8 : params.vix >= 24 ? 4 : 0  // 24 = scoring midpoint, not a gate threshold
  const vixSpikeBonus = params.vixChgPct >= 5 ? 6 : params.vixChgPct >= 3 ? 3 : 0
  const viBonus = params.nikkeiVI >= SHORT_BEAR_NIKKEI_VI_TRIGGER ? 4 : 0
...
function buildShortMode(params: {
  ...
  return {
    ...
    stopLossRule: '-2.8%相当の下落で次回判定時に売却注文（翌営業日以降の基準価額で約定）',
    maxHoldingRule: '最長2営業日（中期保有禁止）',
    invalidationRule: '主要条件のうち2条件以上が崩れたら前提崩れ',
    leveragedWarning: 'ブルベアは短期専用。撤退条件到達で次回判定時に売却注文を検討（即時約定不可）。減価リスクが高いため中期保有は禁止。',
...
function buildShortTermRows(
  ...
  // P4-A146: BUY按分はfloorで安全側に倒し、合計がrecommendedXBudget（≤headroom）を超えないようにする
  const corePerFund =
    coreFunds.length > 0
      ? floorToTenThousand(shortMode.recommendedCoreBudget / coreFunds.length)
      : 0
  const satPerFund =
    satelliteFunds.length > 0
      ? floorToTenThousand(shortMode.recommendedSatelliteBudget / satelliteFunds.length)
      : 0
...
// P4-A147: noTrade中に抑制すべきBUY項目か判定する。
// short-*（当日実行候補）とpolicy-*（運用方針別配分調整）のBUYのみ対象。
// TRIM/EXIT/WATCH/HOLD等の防御・待機系アクションは対象外（弱めない）。
function isNoTradeSuppressedBuyItem(item: TrustExecutionItem): boolean {
  return item.action === 'BUY' && (item.id.startsWith('short-') || item.id.startsWith('policy-'))
}
    ...
    .map(row => ({
      ...
      detail:
        row.action === 'BUY'
          ...
          : row.action === 'EXIT'
          ? 'レバ型の持ち越しを避けるため、当日中に売却注文を優先（翌営業日以降の基準価額で約定）。'
          : row.action === 'TRIM'
...
export function buildTrustPortfolioPlan(input: {
  ...
  // P4-A48: effectiveCoreBudget = min(CORE_BUDGET, max(0, jpTrustHeadroom))
  // headroom未指定時はCORE_BUDGET（現行互換）。WAIT/noTradeでrecommendedが0の場合もmin(0,x)=0で維持。
  const effectiveHeadroom = typeof input.jpTrustHeadroom === 'number'
    ? Math.max(0, input.jpTrustHeadroom)
    : null
  const effectiveCoreBudget = effectiveHeadroom !== null
    ? Math.min(CORE_BUDGET, effectiveHeadroom)
    : CORE_BUDGET
  // P4-A146: satellite予算もheadroom残余（headroom - core配分）でcapする。
  // headroom未指定時はSATELLITE_BUDGET（現行互換）。effectiveCoreBudgetを先に差し引くことで
  // core+satelliteの合計上限がeffectiveHeadroomを超えないことを保証する。
  const effectiveSatelliteBudget = effectiveHeadroom !== null
    ? Math.min(SATELLITE_BUDGET, Math.max(0, effectiveHeadroom - effectiveCoreBudget))
    : SATELLITE_BUDGET
  const shortModeWithBudget: TrustShortMode = {
    ...shortMode,
    recommendedCoreBudget: Math.min(shortMode.recommendedCoreBudget, effectiveCoreBudget),
    recommendedSatelliteBudget: Math.min(shortMode.recommendedSatelliteBudget, effectiveSatelliteBudget),
  }

  const targetRatios = resolvePolicyTargets(input.market, shortModeWithBudget)
  const policyRows = buildPolicyRows(input.trust, targetRatios, shortModeWithBudget)
  const shortTermRows = buildShortTermRows(input.trust, shortModeWithBudget)
  const rawQueue = buildExecutionQueue(shortModeWithBudget, policyRows, shortTermRows)
  // Case C: noTrade gate — suppress new BULL/BEAR short entries, keep TRIM/EXIT/WAIT
  const executionQueue: TrustExecutionItem[] = input.noTrade
    ? [
        {
          id: 'short-header',
          title: '今日の判断: 待機（緊急ノートレード）',
          detail: '市場ボラティリティが高いため、新規短期投信エントリーを停止。既存ポジションの整理（TRIM/EXIT）のみ許可。',
          priority: 'high',
          action: 'WAIT',
        },
        ...rawQueue.filter(
          item =>
            item.id !== 'short-header' &&
            item.action !== 'BULL' &&
            item.action !== 'BEAR' &&
            !isNoTradeSuppressedBuyItem(item),
        ),
      ]
    : rawQueue
