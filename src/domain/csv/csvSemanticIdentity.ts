...
export function stableSerializeCsvSemanticContent(value: unknown): string {
  ...
  return `{${Object.keys(record).sort().map(key =>
    `${JSON.stringify(key)}:${stableSerializeCsvSemanticContent(record[key])}`).join(',')}}`
}

export function fingerprintLegacyCsvSemanticContent(value: unknown): string {
  const serialized = stableSerializeCsvSemanticContent(value)
  let hash = 0x811c9dc5
