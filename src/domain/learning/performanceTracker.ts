...
const MIN_EVAL_INTERVAL_MS = 4 * 60 * 60 * 1000  // 4時間: 同一セッション内の trivial 評価を防止
...
function evaluateOutcomes(
  ...
  const evalTime = new Date(at).getTime()

  prevBaseline.forEach(seed => {
    const h = holdings.find(x => x.code === seed.code)
    if (!h) return

    // 予測から4時間未満の評価はスキップ（delta=0によるHOLD精度100%問題を回避）
    const elapsed = evalTime - new Date(seed.predictedAt).getTime()
    if (elapsed < MIN_EVAL_INTERVAL_MS) return
    ...
    outcomes.push({
      ...
      regime: seed.regime,  // Phase 8: 予測時のレジームを引き継ぐ
    })
  })
...
export function updatePerformanceTracker(
  ...
  const nextBaseline = buildBaseline(holdings, analysis, at, regime)
