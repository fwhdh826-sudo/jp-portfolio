"""
R2 Intraday Routine — Card 2-5
Lightweight Operation Health Check. Runs at intraday_patch time (12:30 JST).
...
Scope: primarily Tier 1 sources (market / regime / news).
Notification trigger: watchdog.system_error OR freshness.safe_mode_triggered.
...
def run_intraday_routine(
    ...
    *,
    ...
    """Run R2 Intraday Routine: lightweight Tier 1 health check.

    Detection-only: no trades, no securities API calls.
    Notification triggered when system_error=True or freshness.safe_mode_triggered=True.
    ...
    # ── 2. Discord notification: trigger = system_error OR freshness stale ────
    notify_result: Optional[NotifyResult] = None
    if notifier_config is not None and (watchdog.system_error or freshness.safe_mode_triggered):
    ...
    recovery_entry = build_routine_recovery_entry(
        ...
        watchdog_critical=watchdog.system_error,  # intraday monitors system_error only
        label="Intraday",
        action_watchdog="intraday_watchdog_alert_raised",
        action_safe_mode=f"restrictions: new_buys_frozen={safe_mode.restrictions.new_buys_frozen}",
    )
