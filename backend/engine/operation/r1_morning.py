"""
R1 Morning Routine — Card 2-5
Full Operation Health Check. Runs at full_batch time (06:30 JST).
...
Discord sending requires notifier_config.dry_run=False from the caller.
...
def run_morning_routine(
    ...
    *,
    ...
    """Run R1 Morning Routine: full Operation Health Check.

    Detection-only: no trades, no securities API calls.
    Notification sent only if notifier_config is provided.
    ...
    # ── 2. Discord notification (if config provided and issues detected) ───────
    ...
    recovery_entry = build_routine_recovery_entry(
        ...
        label="Morning",
        action_watchdog="morning_watchdog_alert_raised",
        action_safe_mode=(
